...
contract('TrainDelay', async (accounts) => {
    ...
    it("should create application and accept multiple policies per trip per account", async () => {
        ...
        let tx1 = await trainDelay.applyForPolicy(web3.utils.asciiToHex('IC123'), 10000, timestamp_1, timestamp_2, 50000, 60, {
            from: HOLDER_1,
    ...
    it("should process claims", async () => {
        let tx1 = await trainDelay.applyForPolicy(web3.utils.asciiToHex('THALYS1234'), 10000, timestamp_1, timestamp_2, 50000, 60, {
            from: HOLDER_2,
            value: web3.utils.toWei('100', 'finney')
        });
        let tripId;
